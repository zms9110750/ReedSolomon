...
public class ReedSolomonEncodeStream : SpanCapableStreamBase
{
    ...
    public ReedSolomonEncodeStream(IMatrix encodingMatrix, StreamRoundRobin parityStreams, Stream? originalStream=null, bool leaveOpen = false)
    {
    ...
    /// <param name="originalStream">原始数据输出流</param>
    /// <param name="leaveOpen">释放对象后，true 将流对象保持打开状态;false将一起释放底层流</param> 
    public ReedSolomonEncodeStream(IMatrix encodingMatrix, IReadOnlyList<Stream> parityStreams, int blockSize, Stream? originalStream, bool leaveOpen = false)
        : this(encodingMatrix, new StreamRoundRobin(parityStreams, blockSize), originalStream, leaveOpen)
    {
    }
    ...
    /// <param name="leaveOpen">释放对象后，true 将流对象保持打开状态;false将一起释放底层流</param> 
    public ReedSolomonEncodeStream(IMatrix encodingMatrix, IReadOnlyList<Stream> parityStreams, StreamRoundRobin dataRoundRobin, bool leaveOpen = false)
        : this(encodingMatrix, new StreamRoundRobin(parityStreams, dataRoundRobin?.SegmentSize ?? throw new ArgumentNullException(nameof(dataRoundRobin))), dataRoundRobin, leaveOpen)
    {
    }
    /// <inheritdoc/>
    ...
    public override async ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        await Pipe.Writer.WriteAsync(buffer, cancellationToken).ConfigureAwait(false);
        _position += buffer.Length;
